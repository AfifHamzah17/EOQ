...
const generateNextSalesNo = async (cabang) => {
  ...
  const lastSale = await SalesModel.findOne({ namaCabang: cabang }).sort({ createdAt: -1 }).select('salesNo').limit(1);
...
export const createSale = async (data, user) => {
  ...
  const existDate = await SalesModel.findOne({ date: data.date, namaCabang });
  if (existDate) throw new Error(`Cabang ${namaCabang} sudah memiliki data penjualan di tanggal tersebut.`);
  const salesNo = await generateNextSalesNo(namaCabang);
  return await SalesModel.create({ ...data, salesNo, namaCabang });
};

export const getAllSales = async (user) => { return await SalesModel.find(getCabangFilter(user)).sort({ createdAt: -1 }); };
...
export const updateSale = async (id, data, user) => {
  ...
  if (!sale) throw new Error('Data tidak ditemukan');
  if (user.role !== 'admin' && sale.namaCabang !== user.namaCabang) throw new Error('Akses ditolak');
  await SalesModel.findByIdAndUpdate(id, { date: data.date, remainingMoney: parseInt(data.remainingMoney) || 0, expense: parseInt(data.expense) || 0, totalAll: parseInt(data.totalAll) || 0, serba35: parseInt(data.serba35) || 0, serba50: parseInt(data.serba50) || 0, serba75: parseInt(data.serba75) || 0 });
...
export const uploadSalesCsv = async (dataArray, user) => {
  const results = [];
  for (const row of dataArray) { try { results.push({ error: false, message: 'Berhasil', data: await createSale(row, user) }); } catch (err) { results.push({ error: true, message: err.message, data: row }); } }
  return { message: 'Proses upload selesai', results };
};

export const bulkDeleteSales = async (ids) => {
  const result = await SalesModel.deleteMany({ _id: { $in: ids } });
  if (!result) throw new Error('Gagal menghapus data');
  return { message: `${result.deletedCount} data penjualan berhasil dihapus` };
};